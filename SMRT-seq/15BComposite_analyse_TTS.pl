...
open (OUT1, ">", "$ARGV[0]_TTS_bin_distribution");
{
		@GFF= <IN1>;
		close IN1;
		@bed= <IN2>;
...
foreach $sequence(@bed){
		...
		foreach $anotation(@GFF){
			if ($anotation=~/$chr.+?gene\t(\d+)\t(\d+)\t(.+?)\t(.)/){
				$oritation=$4;
				if ($oritation eq "+"){$TSS=$2;$bin_size=$start-$TSS;if($bin_size<2001 && $bin_size>-2001){$bin_number{$bin_size}++;}}
				elsif ($oritation eq "-"){$TSS=$1;$bin_size=$TSS-$start;if($bin_size<2001 && $bin_size>-2001){$bin_number{$bin_size}++;}}
				}
			}
